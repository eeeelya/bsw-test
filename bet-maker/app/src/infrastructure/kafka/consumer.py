...
class AIOKafkaConsumerWrapper:
    ...
    @staticmethod
    async def process_message(message: ConsumerRecord, _id: UUID) -> Any:
        raise NotImplementedError

    async def _consume_one_message(self, _id: UUID) -> Any:
        ...
        try:
            return await self.process_message(message, _id)
    ...
    async def consume(self, _id: UUID) -> Any:
        if self._consumer is None:
            raise KafkaDidNotStarted

        try:
            return await self._consume_one_message(_id)
        finally:
